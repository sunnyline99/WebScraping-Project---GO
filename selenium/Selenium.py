...
if gecko =='':
    ...
    print("Please check the path to geckodriver and make sure you provided a good one.")
else:
    gecko = gecko.replace('"', '')
    gecko_path = gecko
...
data = pd.DataFrame({'player_name':[], 'date_of_birth':[], 'current_elo':[], 'wins':[], 'losses':[], 'total':[], 'white_win_perc':[],'white_lose_perc':[], 'black_win_perc':[], 'black_lose_perc':[], 'average_rating_differences':[]})
...
for i in range(2,no_pages+2):
    ...
    player = {'player_name':player_name,'date_of_birth':player_date_of_birth, 'current_elo':player_current_elo, 'wins':player_wins, 'losses':player_losses, 'total':player_total, 'white_win_perc':ww_perc, 'white_lose_perc':wl_perc, 'black_win_perc':bw_perc, 'black_lose_perc':bl_perc, 'average_rating_differences':av_diff}
...
if path =='':
    file_path=""
    data.to_csv('Go_Players_Data_Selenium.csv')
else:
