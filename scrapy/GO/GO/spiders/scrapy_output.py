...
class Info(scrapy.Item): #create scrapy fields
    ...
    white_win_perc = scrapy.Field()
    white_lose_perc = scrapy.Field()
    black_win_perc = scrapy.Field()
    black_lose_perc = scrapy.Field()
    current_elo= scrapy.Field()
...
#between the player's and the opponents ratings
        ...
        p['player_name'] = response.xpath(player_xpath).getall()
        p['date_of_birth'] = response.xpath(date_xpath).getall()
        p['wins'] = response.xpath(wins_xpath).getall()
        p['losses'] = response.xpath(losses_xpath).getall()
        p['white_win_perc'] = ww/10 #calculate the instances as a proportion of total games taken into accouy
        p['white_lose_perc'] = wl/10
        p['black_win_perc'] = bw/10
        p['black_lose_perc'] = bl/10
        p['current_elo'] =response.xpath(rating_xpath).getall()
        ...
        yield p
# TERMINAL: scrapy crawl scrapy_output -o Go_Players_Data_Scrapy.csv -t csv